...
SWEP.Type                   = ( TFA and TFA.Yankys_Custom_Weapon_Pack ) and "Type: Bolt-Action Sniper Rifle - Caliber: .338 Lapua Magnum - Muzzle Velocity: 1005 m/s" or "Bolt-Action Sniper Rifle"
SWEP.Purpose				= "A Multi-Caliber High Capacity Bolt Action Sniper Rifle."   
SWEP.Instructions		    = ""    
...
SWEP.Primary.Sound          = Sound("TFA_INS2.PD2_MSR.Fire")            -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound  = Sound("TFA_INS2.PD2_MSR.Fire_Suppressed") -- This is the sound of the weapon, when silenced.
...
SWEP.Primary.Damage                = ( TFA and TFA.Yankys_Custom_Weapon_Pack ) and 185 * TFA.Yankys_Custom_Weapon_Pack.DamageMultiplier or 185
...
SWEP.Primary.Velocity              = 1005                     -- Bullet Velocity in m/s
...
-- Firing Cone Related
SWEP.Primary.Spread              = .033                       -- This is hip-fire acuracy.  Less is more (1 is horribly awful, .0001 is close to perfect)
SWEP.Primary.IronAccuracy        = .0001                      -- Ironsight accuracy, should be the same for shotguns
SWEP.IronSightTime               = 0.43                       -- The time to enter ironsights/exit it.

-- Unless you can do this manually, autodetect it.  If you decide to manually do these, uncomment this block and remove this line.
SWEP.Primary.SpreadMultiplierMax = 5.8                        -- How far the spread can expand when you shoot. Example val: 2.5
...
-- Range Related
SWEP.Primary.Range               = 1.8 * (3280.84 * 16)       -- The distance the bullet can travel in source units.  Set to -1 to autodetect based on damage/rpm.
SWEP.Primary.RangeFalloff        = 0.88                       -- The percentage of the range the bullet damage starts to fall off at.  Set to 0.8, for example, to start falling off after 80% of the range.
...
SWEP.VMPos                 = ( TFA and TFA.Yankys_Custom_Weapon_Pack ) and Vector(-1.35, -0.25, 0.1) or Vector(-0.65, -0.25, 0.1)
...
SWEP.SequenceLengthOverride = {                               -- Changes both the status delay and the nextprimaryfire of a given animation
	[ACT_VM_PULLBACK_LOW]   = 30.5 / 30,
	[ACT_VM_PULLBACK_HIGH]  = 30.5 / 30,
}   
...
SWEP.EventTable = {
	[ACT_VM_PULLBACK_LOW] = {
		{ ["time"] = 14.5 / 39, ["type"] = "lua", ["value"] = function(wep,vm)
	...
	[ACT_VM_PULLBACK_HIGH] = {
		{ ["time"] = 18.1 / 39, ["type"] = "lua", ["value"] = function(wep,vm)
			if wep and wep.EventShell then
				wep:EventShell()
			end
		end, ["client"] = true, ["server"] = true }
	...
	[ACT_VM_RELOAD_EMPTY] = {
		{ ["time"] = 22.15 / 30, ["type"] = "lua", ["value"] = function(wep,vm)
			if wep and wep.EventShell then
				wep:EventShell()
			end
		end, ["client"] = true, ["server"] = true }
...
SWEP.WorldModel	   = "models/weapons/w_ins2_pd2_msr.mdl" -- Weapon world model path
...
SWEP.IronSightsPos     = Vector(-3.578, -5, 0.215)
SWEP.IronSightsAng     = Vector(0, 0.045, 0)
...
SWEP.ViewModelBoneMods = {
	...
	["A_Optic"] = { scale = Vector(0.815, 0.815, 0.815), pos = Vector(0.05, -0.1, -0.25), angle = Angle(0, 0, 0) },
	["A_Suppressor"] = { scale = Vector(0.9, 1, 0.9), pos = Vector(0, -0.06, 4), angle = Angle(0, 0, 0) },
	["A_LaserFlashlight"] = { scale = Vector(0.82, 0.82, 0.82), pos = Vector(1.77, 0, 0.3), angle = Angle(0, 0, 180) },

	["body"] = { scale = Vector(1.05, 1.05, 1.05), pos = Vector(-0.5, 0, 0.2), angle = Angle(0, 0, 0) },
	["bolt"] = { scale = Vector(1.05, 1.05, 1.05), pos = Vector(0, 0.1, 0), angle = Angle(0, 0, 0) },
	["bolt001"] = { scale = Vector(1.05, 1.05, 1.05), pos = Vector(0, 0.1, 0), angle = Angle(0, 0, 0) },
	["mag"] = { scale = Vector(1.05, 1.05, 1.05), pos = Vector(0, 0, 0.2), angle = Angle(0, 0, 0) },
	["mag001"] = { scale = Vector(1.05, 1.05, 1.05), pos = Vector(0, 0, 0.2), angle = Angle(0, 0, 0) },
	["bullet"] = { scale = Vector(1.05, 1.05, 1.05), pos = Vector(0, 0, 0.2), angle = Angle(0, 0, 0) },

	["L UpperArm"] = { scale = Vector(1, 1, 1), pos = Vector(0.125, 0.2, -0.1), angle = Angle(0, 0, 0) },
 	["L Finger0"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 1.65, 0) },  
  	["L Finger01"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -0.25, 0) },  
 	["L Finger02"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -0.25, 0) },  
    ["L Finger1"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -11.25, 0) },   
	["L Finger2"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -10.5, 0) },
	["L Finger3"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -10.5, 0) },
	["L Finger4"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, -10.5, 0) },

	["R UpperArm"] = { scale = Vector(1, 1, 1), pos = Vector(0.1, 0, 0.2), angle = Angle(0, 0, 0) },
...
SWEP.WorldModelBoneMods = {
	["ATTACH_ModKit"] = { scale = Vector(0.8, 0.8, 0.8), pos = Vector(1, 0, 0.05), angle = Angle(0, 0, 0) },
	["ATTACH_Eject"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) },
	["ATTACH_Laser"] = { scale = Vector(1, 1, 1), pos = Vector(4, -0.65, 0.15), angle = Angle(0, 0, -180) },
}

SWEP.VElements = {
	["sights_folded"] = { type = "Model", model = "models/weapons/upgrades/a_standard_remington_msr.mdl", bone = "A_Optic", rel = "", pos = Vector(-3.413, 2.82, -12.6), angle = Angle(90, 90, 0), size = Vector(1.05, 1.05, 1.05), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = true},
	...
	["suppressor_osprey"] = { type = "Model", model = "models/weapons/tfa_eft/upgrades/v_osprey.mdl", bone = "A_Muzzle", rel = "", pos = Vector(-1.5, 0.05, -0.12), angle = Angle(0, -90, 0), size = Vector(1.25, 1.25, 1.25), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} , bonemerge = false, active = false },	
...
SWEP.WElements = {
	...
	["sights_folded"] = { type = "Model", model = "models/weapons/upgrades/a_standard_remington_msr.mdl", bone = "A_Optic", rel = "ref", pos = Vector(-0.65, -4.65, 9), angle = Angle(3, -5.75, -0.5), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = true },
