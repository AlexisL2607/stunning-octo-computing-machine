    ...
    if tonumber(pStats.alive) == 1 then
        ...
        table.foreach( pAmmo, function( key, value )
            if key ~= "unique_id" then
              ...
              --  ply:Say("Ajout de "..value.." de type :".. key)
            end
        end)
    end
end
...
function SpawnBanks()
    for _, vec in pairs(Banks[string.lower(game.GetMap())]) do
        ...
        Bank_Box:SetPos(vec)
        Bank_Box:SetAngles(Angle(0, 90, 0))
        Bank_Box:Spawn()
    end
end
hook.Add("InitPostEntity", "SpawnBanks", SpawnBanks)

function SpawnMedShop()
    for _, vec in pairs(MedShops[string.lower(game.GetMap())]) do
        local Bank_Box = ents.Create("medshop")
        Bank_Box:SetPos(vec + Vector(0, 0, -14))
		Bank_Box:SetAngles(Angle(0, 90, 0))
        Bank_Box:Spawn()
    end
end
hook.Add("InitPostEntity", "SpawnMedShop", SpawnMedShop)
